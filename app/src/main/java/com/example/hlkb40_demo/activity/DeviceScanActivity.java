...
public class DeviceScanActivity extends Activity {
        ...
        if (requestCode == REQUEST_PERMISSION_CODE) {
            ...
            if (grantResults.length > 0) {
                ...
                if (allPermissionsGranted) {
                    Toast.makeText(this, "權限已獲取，開始掃描設備", Toast.LENGTH_SHORT).show();
                    scanLeDevice(true);
                } else {
                    Toast.makeText(this, "需要藍牙和位置權限才能搜索設備。", Toast.LENGTH_LONG).show();
                }
            }
        }
    }
    ...
    protected void onActivityResult(int requestCode, int resultCode, Intent data) {
        if (requestCode == REQUEST_LOCATION_PERMISSION) {
            ...
            if (resultCode == Activity.RESULT_OK) {
                ...
                Toast.makeText(this, "藍牙未開啟，應用程式功能將受限", Toast.LENGTH_SHORT).show();
            }
        }
    ...
    private ScanCallback scanCallback = new ScanCallback() {
        ...
        public void onScanFailed(int errorCode) {
            ...
            Log.e(TAG, "onScanFailed with error code: " + errorCode);
        }
    };
